...
# Restart the PostgreSQL service using docker-compose
cd "$COMPOSE_DIR" && docker compose stop postgres 2>&1 | tee -a "$LOG_FILE"
cd "$COMPOSE_DIR" && docker compose start postgres 2>&1 | tee -a "$LOG_FILE"
